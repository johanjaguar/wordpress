<div class="breadcrumbs container l-white">
<?php
function breadcrumbs() {
	...
	echo 'Estás en <a href="'.$url.'/" rel="home" class="l-white">Inicio</a>';
	$dir = $url.'/';
	if(is_single() || is_category()) {
		...
		foreach($url_array as $folder) {
			if($folder != 'category'){
				...
				echo ' &raquo; <a href="'.$dir.'" rel="category tag" class="l-white">'.ucwords(str_replace("-", " ", $folder)) . '</a>';
			}
		}
	} else {
		...
		foreach($url_array as $folder) {
			if($folder != 'tag' && $folder != 'author'){
				if(!is_numeric($folder)){
					...
					echo ' &raquo; <a href="'.$dir.'" class="l-white">'.ucwords(str_replace("-", " ", $folder)) . '</a>';
				}
			}
		}
	}
