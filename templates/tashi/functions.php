...
return ' <a title="Ver Mas" href="'.get_permalink().'" rel="nofollow" id="vermas">Ver Mas&nbsp;&raquo;</a>';
}
...
if ( function_exists( 'add_image_size' ) ) {
	add_image_size( 'full', 1900, 920, true  );
...
add_action( 'wp_enqueue_scripts', 'custom_frontend_scripts' );function custom_frontend_scripts() {
  global $post, $woocommerce;

  $suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
  wp_deregister_script( 'jquery-cookie' );
  wp_register_script( 'jquery-cookie', $woocommerce->plugin_url() . '/assets/js/jquery-cookie/jquery_cookie' . $suffix . '.js', array( 'jquery' ), '1.4.1', true );
...
function bg_recent_comments($no_comments = 5, $comment_len = 80, $avatar_size = 48) {
  ...
  if ( $comments ) : foreach ( $comments as $comment ) :
    ...
    $comm .= '<li><a class="author opinionName l-mariner" href="' . get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID . '">';

    $comm .= get_comment_author( $comment->comment_ID ) . ':</a></li> ';
    $comm .= '<li><a class="opinionFecha">' . date("d-m-Y", strtotime($comment->comment_date)) . '</a></li>';

    /*<div class="opinionBox aliceblue">
      <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Esse laboriosam praesentium, fugiat, ex qui maiores voluptas est blanditiis iusto ducimus fuga magni molestias, accusantium assumenda ratione ea? Eum dicta ut quaerat maxime ullam autem rerum molestias aspernatur cum, quae similique?</p>
      <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Perspiciatis nemo rem dicta natus cum sequi distinctio perferendis, facilis eos, adipisci suscipit quas alias. Cumque, autem!</p>
      <p class="opinionName l-mariner">Stella Gomez</p>
      <p class="opinionFecha">Ago 06, 2014</p>
    </div>*/
  endforeach; else :
    $comm .= 'No comments.';
  endif;
